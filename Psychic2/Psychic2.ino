...
volatile byte currentState = WAITING_FOR_FIRST_DIGIT;

volatile bool justSayNo;
...
void DisplayCurrentNumber() {
  ...
  if (currentState == WAITING_FOR_FIRST_DIGIT) {
    if (currentNumber == 0) currentDigit = 10; // Dash  
    if (justSayNo) {
      currentDigit = (currentDigitIndex == 0) ? 12 /*o*/ : 11/*N*/; // No!
    }
  }
