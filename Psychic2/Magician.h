...
void SaveSentinel() {
  delay(PAUSE_FOR_EFFECT); // Pause for effect before showing "No"
...
void ProcessSecondDigit(char keypress) {
  ...
  if (keypress >= '0' && keypress <= '9') {
    ...
    Serial.println("Waiting for effect");
    justSayNo = false;
    delay(PAUSE_FOR_EFFECT); // Pause for effect before showing "No" or applause
    justSayNo = true;
    Serial.println("DONE Waiting for effect");
